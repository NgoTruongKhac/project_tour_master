...
export default function FilterTourPage() {
  ...
  const filteredTours = useMemo(() => {
    // 1. Lấy các giá trị từ URL
    const searchKeyword = searchParams.get("search"); // Từ khóa tìm kiếm chung
    const currentDest = searchParams.get("destination"); // Filter dropdown điểm đến
    ...
    return DataTours.filter((tour) => {
      // --- LOGIC 1: XỬ LÝ SEARCH KEYWORD (LIKE) ---
      if (searchKeyword) {
        const keywordClean = removeAccents(searchKeyword);
        const titleClean = removeAccents(tour.title);
        // Tour destination thường là slug "ha-noi", ta replace "-" thành space để search tự nhiên hơn
        const destClean = removeAccents(tour.destination).replace(/-/g, " ");

        // Logic OR: Tìm thấy trong Title HOẶC trong Destination
        const isMatchTitle = titleClean.includes(keywordClean);
        const isMatchDest = destClean.includes(keywordClean);

        if (!isMatchTitle && !isMatchDest) {
          return false; // Không khớp cái nào thì loại
        }
      }

      // --- LOGIC 2: FILTER ĐIỂM ĐẾN (Dropdown Sidebar) ---
      // Logic này chạy song song. Nếu user vừa search "Biển" vừa chọn Filter Destination "Nha Trang"
      // thì tour phải thỏa mãn CẢ HAI.
      if (
        currentDest &&
        currentDest !== "all" &&
      ...
      // --- LOGIC 4: KHỞI HÀNH ---
      if (currentDeparture && currentDeparture !== "all") {
        // So sánh tương đối phòng trường hợp data viết hoa/thường khác nhau
        if (
          !removeAccents(tour.departurePoint).includes(
            removeAccents(currentDeparture)
          )
        ) {
          return false;
        }
      ...
      // --- LOGIC 6: PHƯƠNG TIỆN ---
      if (currentTransport) {
        const transportLower = removeAccents(tour.transportation);
        if (
          currentTransport === "maybay" &&
          !transportLower.includes("may bay")
        )
          return false;
              ...
              >
                ...
                <div className="flex flex-col justify-between flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between pt-3 mt-3 border-t border-gray-200">
                    ...
                    <div>
                      <button className="px-4 py-2 text-sm font-medium border rounded-lg border-primary text-primary hover:bg-primary hover:text-white focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                        Xem chi tiết
                      </button>
                    </div>
                  </div>
